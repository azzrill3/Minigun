...
{
    public class CoDLTD : ModProjectile
    {
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Double Spark");
		}
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            if (_charge == 100)
            {
                ...
                unit.Normalize();
                DrawLaser(spriteBatch, Main.projectileTexture[projectile.type], Main.player[projectile.owner].Center, unit, 5, projectile.damage, -1.57f, 1f, 1000f, Color.White, 115);// this is the projectile sprite draw, 45 = the distance of where the projectile starts from the player
            }
            return false;
        ...
        public void DrawLaser(SpriteBatch spriteBatch, Texture2D texture, Vector2 start, Vector2 unit, float step, int damage, float rotation = 0f, float scale = 1f, float maxDist = 2000f, Color color = default(Color), int transDist = 50)
        {
            ...
            for (float i = transDist; i <= _moveDist; i += step)
            {
                ...
                spriteBatch.Draw(texture, origin - Main.screenPosition,
                    new Rectangle(0,208 , 204, 208), i < transDist ? Color.Transparent : c, r,
                    new Vector2(204 / 2 , 208 / 4), scale, 0, 0);
            }
            #endregion
 
            #region Draw laser tail
            spriteBatch.Draw(texture, start + unit * (transDist - step) - Main.screenPosition,
                new Rectangle(0, 0, 204, 208), Color.White, r, new Vector2(204 / 2, 208 / 2), scale, 0, 0);
            #endregion
 
            #region Draw laser head
            spriteBatch.Draw(texture, start + (_moveDist + step) * unit - Main.screenPosition,
                new Rectangle(0, 416, 204, 208), Color.White, r, new Vector2(204 / 2, 208 / 2), scale, 0, 0);
            #endregion
        }
        ...
        public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox)
        {
            if (_charge == 100)
            {
                ...
                unit.Normalize();
                float point = 0f;
                if (Collision.CheckAABBvLineCollision(targetHitbox.TopLeft(), targetHitbox.Size(), p.Center - 10f * unit, p.Center - unit * _moveDist, 200, ref point))
                {
                    return true;
                }
            }
