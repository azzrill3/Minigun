...
{
    public class KRock : ModProjectile
    {
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Karos Beam");
		}
        private Vector2 _targetPos;         //Ending position of the laser beam
        private int _charge;                //The charge level of the weapon
        private float _moveDist = 90f; 		//The distance charge particle from the player center
		private Vector2 laserY;
 ...
 //this is the projectile name
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            if (_charge == 100)
            {
				Vector2 laserY = new Vector2(-68,-68);
                Vector2 unit = _targetPos - Main.player[projectile.owner].Center;
				Vector2 laserX = -unit + unit;
                unit.Normalize();
                DrawLaser(spriteBatch, Main.projectileTexture[projectile.type], Main.player[projectile.owner].Center, unit, 5, projectile.damage, -1.57f, 1f, 1000f, Color.White, 68);// this is the projectile sprite draw, 45 = the distance of where the projectile starts from the player
				DrawLaser(spriteBatch, Main.projectileTexture[projectile.type], Main.player[projectile.owner].Center, -unit, 5, projectile.damage, -1.57f, 1f, 1000f, Color.White, 68);
				//DrawLaser(spriteBatch, Main.projectileTexture[projectile.type], Main.player[projectile.owner].Center, laserX, 5, projectile.damage, -1.57f, 1f, 1000f, Color.White, 68);
            }
            return false;
        ...
        public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox)
        {
            if (_charge == 100)
            {
				...
				else if (Collision.CheckAABBvLineCollision(targetHitbox.TopLeft(), targetHitbox.Size(), p.Center - 95f * unit, p.Center - unit * _moveDist, 22, ref point))
                {
                    return true;
                }
            }
