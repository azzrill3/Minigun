...
{
    public class CoDLTF : ModProjectile
    {
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Final Master Spark");
		}
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            if (_charge == 100)
            {
                ...
                unit.Normalize();
                DrawLaser(spriteBatch, Main.projectileTexture[projectile.type], Main.player[projectile.owner].Center, unit, 5, projectile.damage, -1.57f, 1f, 1000f, Color.White, 220);// this is the projectile sprite draw, 45 = the distance of where the projectile starts from the player
            }
            return false;
        ...
        public void DrawLaser(SpriteBatch spriteBatch, Texture2D texture, Vector2 start, Vector2 unit, float step, int damage, float rotation = 0f, float scale = 1f, float maxDist = 2000f, Color color = default(Color), int transDist = 50)
        {
            ...
            for (float i = transDist; i <= _moveDist; i += step)
            {
                ...
                spriteBatch.Draw(texture, origin - Main.screenPosition,
                    new Rectangle(0,416 , 408, 416), i < transDist ? Color.Transparent : c, r,
                    new Vector2(408 / 2 , 416 / 4), scale, 0, 0);
            }
            #endregion
 
            #region Draw laser tail
            spriteBatch.Draw(texture, start + unit * (transDist - step) - Main.screenPosition,
                new Rectangle(0, 0, 408, 416), Color.White, r, new Vector2(408 / 2, 416 / 2), scale, 0, 0);
            #endregion
 
            #region Draw laser head
            spriteBatch.Draw(texture, start + (_moveDist + step) * unit - Main.screenPosition,
                new Rectangle(0, 832, 408, 416), Color.White, r, new Vector2(408 / 2, 416 / 2), scale, 0, 0);
            #endregion
        }
        ...
        public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox)
        {
            if (_charge == 100)
            {
                ...
                unit.Normalize();
                float point = 0f;
                if (Collision.CheckAABBvLineCollision(targetHitbox.TopLeft(), targetHitbox.Size(), p.Center - 10f * unit, p.Center - unit * _moveDist, 400, ref point))
                {
                    return true;
                }
            }
        ...
        public override void AI()
        {
            ...
            else
            {
                ...
                for (int k = 0; k < chargeFact + 1; k++)
                {
                    ...
                    Dust dust = Main.dust[Dust.NewDust(dustPos, 30, 30, 14, projectile.velocity.X / 2f,    //this 30, 30 is the dust weight and height 235 is the tail dust    
                        projectile.velocity.Y / 2f, 255,new Color(), 1f)];
                    dust.velocity = Vector2.Normalize(spawnPos - spawn) * 3f * (10f - chargeFact * 2f) / 1f;
                    dust.noGravity = true;
                    dust.scale = Main.rand.Next(10, 20) * 0.25f;
                }
            }
            #endregion
            ...
            for (int i = 0; i < 2; ++i)
            {
                ...
                Dust dust = Main.dust[Dust.NewDust(_targetPos, 0, 0, 14, dustVel.X, dustVel.Y, 255, new Color(), 1f)];  //this is the head dust
                Dust dust2 = Main.dust[Dust.NewDust(_targetPos, 0, 0, 14, dustVel.X, dustVel.Y, 255, new Color(), 1f)]; //this is the head dust 2
                dust.noGravity = true;
                dust.scale = 1.2f;
            }
