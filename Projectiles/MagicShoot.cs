...
{
    public class MagicShoot : ModProjectile
    {
        ...
        private float _moveDist = 190f;       //The distance charge particle from the player center
    ...
    //this is the projectile name
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            if (_charge == 0)
            {
				...
				Vector2 pos = Main.player[projectile.owner].Center - offset;
                Vector2 unit = _targetPos - Main.player[projectile.owner].Center;
                unit.Normalize();
                DrawLaser(spriteBatch, Main.projectileTexture[projectile.type], pos, unit, 5, projectile.damage, -1.57f, 1f, 1000f, Color.White, 140);// this is the projectile sprite draw, 45 = the distance of where the projectile starts from the player
            }
            return false;
        ...
        public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox)
        {
            if (_charge == 0)
            {
                ...
                unit.Normalize();
                float point = 0f;
                if (Collision.CheckAABBvLineCollision(targetHitbox.TopLeft(), targetHitbox.Size(), p.Center - 10f * unit, p.Center - unit * _moveDist, 22, ref point))
                {
                    return true;
                }
            }
        ...
        public override void AI()
        {
			...
			else
            {
                ...
                for (int k = 0; k < chargeFact + 1; k++)
                {
                    ...
                    Dust dust = Main.dust[Dust.NewDust(dustPos, 30, 30, 272, projectile.velocity.X / 2f,    //this 30, 30 is the dust weight and height 235 is the tail dust    
                        projectile.velocity.Y / 2f, 0,new Color(255,0,0), 1f)];
            ...
            for (int i = 0; i < 2; ++i)
            {
                ...
                Dust dust = Main.dust[Dust.NewDust(_targetPos, 0, 0, 272, dustVel.X, dustVel.Y, 0, new Color(), 1f)];  //this is the head dust
                Dust dust2 = Main.dust[Dust.NewDust(_targetPos, 0, 0, 272, dustVel.X, dustVel.Y, 0, new Color(), 1f)]; //this is the head dust 2
                dust.noGravity = true;
                dust.scale = 1.2f;
            }
			if(_charge == 100) {player.channel = false;}
